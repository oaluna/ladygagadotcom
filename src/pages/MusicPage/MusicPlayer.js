...
import ChromaticaData from "./chromatica.js"

class MusicPlayer extends Component {
  constructor(props) {
    super(props);
    this.state = {
      albumData: ChromaticaData,
    ...
    this.firePlayingStatusChangeEvent =
      this.firePlayingStatusChangeEvent.bind(this);
  }
  ...
  fetchAlbumData() {
    ...
    var that = this;
    var queryString =
      "?q=album:" + this.state.albumName + " artist:" + this.state.artistName;
  ...
  componentWillUpdate(newProps) {
    if (
      newProps.albumName !== this.props.albumName ||
      newProps.artistName !== this.props.artistName
    ) {
      this.setState(
        { artistName: newProps.artistName, albumName: newProps.albumName },
        this.fetchAlbumData
      );
    }
  }
        ...
        return (
          <div className="media">
            ...
            <div className="media-body">
              <h4 className="media-heading">{this.state.albumData.name}</h4>
    ...
    if (this.state.albumData === null) {
      return (
        <section className="spotify-player">
          <div
            dangerouslySetInnerHTML={{ __html: this.state.playerPlaceholder }}
          ></div>
        </section>
      );
    } else {
